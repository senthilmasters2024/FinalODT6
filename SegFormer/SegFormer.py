...
class _ROS2Publisher(Node):
    ...
    def publish(self, detections, annotated_bgr, depths):
        """
        detections   : list of dicts with 'label', 'confidence', 'bbox' (list [x1,y1,x2,y2])
    ...
    def __init__(
        ...
        if use_ros:
            if not ROS2_AVAILABLE:
                ...
                rclpy.init()
                self._ros_pub = _ROS2Publisher('segformer_detector', '/segformer')
                print("ROS 2 publisher ready on /segformer/*")
        ...
        try:
            while True:
                ...
                if self._ros_pub is not None:
                    depths = [d['depth'] for d in last_detections]
...
def main():
    ...
    print("="*60)
    if use_ros:
        print("  ROS 2 output : /segformer/detections | /segformer/detection_depths | /segformer/annotated_image")
